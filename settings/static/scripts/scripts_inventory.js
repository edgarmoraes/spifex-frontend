...
function resetModalFields(formSelector) {
  ...
  document.querySelectorAll(".modal-delete-inventory").forEach(button => button.style.display = 'none');
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const modalConfig = {
    openBtn: '.add-inventory', 
    modal: '.inventory-modal', 
    form: '.inventory-modal-form', 
    config: {closeBtnSelector: '.modal-close-inventory'}
  };
...
document.addEventListener('DOMContentLoaded', function() {
  // Seleciona todas as linhas da tabela de inventário
  const inventoryRows = document.querySelectorAll('.inventory-table__body--row');

  // Adiciona o evento de duplo clique em cada linha da tabela de inventário
  inventoryRows.forEach(row => {
    row.addEventListener('dblclick', function() {
      ...
      // Preenche o campo 'inventory' com o nome do item de inventário clicado
      const inventoryItem = this.querySelector('.inventory-item-row').textContent;
      document.querySelector('#inventory-item').value = inventoryItem;

      // Preenche os campos ocultos com os dados da linha selecionada
      document.querySelector('input[name="inventory_item_id"]').value = this.dataset.idInventoryItem;
      document.querySelector('input[name="inventory_item_code"]').value = this.dataset.inventoryItemCode;
      document.querySelector('input[name="inventory_quantity"]').value = this.dataset.inventoryQuantity;
      document.querySelector('input[name="inventory_item_uuid"]').value = this.dataset.uuidInventoryItem;

      // Torna o botão de excluir inventário visível
      document.querySelector('.modal-delete-inventory').style.display = 'block';
    });
  });

  // Evento para fechar o modal ao clicar no botão 'Cancelar'
  document.querySelector('.modal-close-inventory').addEventListener('click', function() {
    document.querySelector('.inventory-modal').close();  // Fecha o modal usando close se estiver usando <dialog>
  });
});

// Configurações para adicionar, atualizar ou apagar um item de inventário
// Esta função lida com o envio do formulário do modal e a exclusão de itens de inventário.
document.addEventListener('DOMContentLoaded', function() {
  const inventoryForm = document.querySelector('.inventory-modal-form');
  const csrftoken = document.querySelector('[name=csrfmiddlewaretoken]').value; // Obtém o token CSRF para a requisição

  // Evento para o envio do formulário de inventário
  inventoryForm.addEventListener('submit', function(e) {
    e.preventDefault();

    const formData = new FormData(this);  // Cria um FormData com os dados do formulário
    fetch('/configuracoes/salvar_estoque/', {
      method: 'POST',
      body: formData,
  ...
  const deleteButton = document.querySelector('.modal-delete-inventory');
  // Evento para a exclusão de um item de inventário
  deleteButton.addEventListener('click', function() {
    const inventoryId = document.querySelector('input[name="inventory_item_id"]').value;  // Obtém o ID do item de inventário a ser excluído
    if (inventoryId) {
      fetch(`/configuracoes/verificar_e_excluir_estoque/${inventoryId}/`, {
        method: 'POST',
        headers: {
      ...
      .then(response => response.json())
      .then(data => {
        alert(data.success ? 'Item excluído com sucesso.' : 'Erro ao excluir item.');  // Mostra uma mensagem com o resultado da operação
        if(data.success) {
          window.location.reload();  // Recarrega a página para remover o item de inventário excluído
        }
      })
      .catch(error => console.error('Erro:', error));
    }
